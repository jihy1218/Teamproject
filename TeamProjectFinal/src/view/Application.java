...
public class Application {
	...
	public static void main(String[] args) { 
		...
		Thread thread = new FileThread(); // 파일스레드 객체 생성
		thread.start(); // 파일 스레드 실행
		...
		try {
			while (true) {
				...
				if (ch == 1) { // 잔여백신확인 전체리스트가 나와야함 처음에는 비어있을수있음
					...
					System.out.println("선택할 백신명 : ");
					String v_name = scanner.next();
					System.out.println("선택할 지역명 : ");
					String v_area = scanner.next();
					...
					if (m_check == false) {
						System.out.println("예약 되어있지않습니다");
					}
				} else if (ch == 4) { // 신청취소
			...
			System.out.println("[[알림]] : 예약자 메뉴 오류입니다.");
		...
		while (true) {
			try {
				...
				if (ch == 1) { // 등록
					...
					System.out.println("[[알림]] : 관리자 메뉴 오류입니다. ");
					scanner = new Scanner(System.in);
				}
